...
nano-vllm 目前仅支持 Qwen3。LLaMA 3 与 Qwen3 架构 95% 相同：
...
| 激活函数 | SiLU | SiLU（相同） |
| 注意力 | GQA | GQA（相同） |
| Q/K Norm | 仅 qkv_bias=False 时 | 无（qkv_bias 始终 True） |
| MLP 结构 | gate+up fused / down | gate+up fused / down（相同） |
| packed_modules_mapping | 同 | 同 |

唯一差异是 LLaMA 不需要 Q/K-Norm，而现有 Qwen3 代码中用 `if not qkv_bias: q, k = q_norm(q), k_norm(k)` 天然路径区分 — LLaMA 的 `qkv_bias=True` 会自动跳过 Q/K Norm。

推荐首个新增模型：**`meta-llama/Llama-3.2-1B`**（1B 参数，RTX 4090 轻松运行）。
...
├── qwen3.py     # ~20 行，继承 base，保留原有类名
└── llama.py     # ~20 行，继承 base，类名映射到 Llama
```

**为什么这是兼容多模型的写法：**

1. `base.py` 不是"Qwen3 基类"，而是 **Llama 家族** 的抽象。LLaMA、Qwen2/3、Mistral、Yi、DeepSeek 等一大类模型共享这个结构。`DecoderLM`、`DecoderLayer`、`DecoderAttention` 等命名反映了这一点。

2. 每个具体模型文件是薄包装器，提供两个能力：
   - **清晰的可读性** — `LlamaForCausalLM(hf_config)` 比 `DecoderLM(hf_config)` 表意
   - **模型差异的覆写点** — 比如将来到 Mistral（有 sliding window attention），只需在 `MistralAttention` 中覆写 `forward`，不改基类

3. 模型分发用 if/elif 而非注册表。两个模型不配 registry，后续到 4+ 可升级（Rule of Three）。

4. `layers/` 完全模型无关 — Attention、Linear、RMSNorm、RoPE 等比基类更底层，LLaMA 和 Qwen3 共用。

**架构演进示意：**

```
新增 LLaMA:   base.py (已有 6 类) ← qwen3.py (wrapper) + llama.py (wrapper)
新增 Mistral: base.py 不动           ← mistral.py (wrapper，覆写 Attention.forward)
新增 Gemma:   base.py 需改           ← GeGLU 替代 SiLU，需改 MLP
新增 GPT:     base.py 不够用          ← LayerNorm 替代 RMSNorm，需大改
```
...
1. **创建 `nanovllm/models/base.py`**
   - 从 `qwen3.py` 复制 6 个类，重命名：
     - `Qwen3Attention` → `DecoderAttention`
     - `Qwen3MLP` → `DecoderMLP`
     - `Qwen3DecoderLayer` → `DecoderLayer`
     - `Qwen3Model` → `DecoderModel`
     - `Qwen3ForCausalLM` → `DecoderLM`（含 `packed_modules_mapping`）
   - Config 类型标注改为 `PretrainedConfig`（from transformers）
   - 零依赖 `qwen3.py`

2. **重构 `nanovllm/models/qwen3.py`**
   - 替换为从 `base.py` 继承的薄包装，保留原类名：
     ```python
     from nanovllm.models.base import *
     class Qwen3Attention(DecoderAttention): pass
     class Qwen3MLP(DecoderMLP): pass
     class Qwen3DecoderLayer(DecoderLayer): pass
     class Qwen3Model(DecoderModel): pass
     class Qwen3ForCausalLM(DecoderLM): pass
     ```
...
4. **创建 `nanovllm/models/llama.py`**（~20 行，同薄包装模式）

5. **修改 `nanovllm/engine/model_runner.py`**
   - 删除硬编码 import，新增 `_load_model(hf_config)`：
     ```python
...
6. **创建 `tests/test_llama.py`** — 冒烟测试：加载模型、生成、验证输出形状和非空、确定性
7. **已有测试回归**（Qwen3 侧全部不动）
8. **手动冒烟** — `example.py` 指向 LLaMA 路径跑一次

### 涉及文件

| 文件 | 操作 | 说明 |
|------|------|------|
| `nanovllm/models/base.py` | 新建 | 通用架构 |
| `nanovllm/models/qwen3.py` | 修改 | 内联 → wrapper |
| `nanovllm/models/llama.py` | 新建 | LLaMA wrapper |
| `nanovllm/engine/model_runner.py` | 修改 | 硬编码 → 分发 |
| `tests/test_llama.py` | 新建 | 冒烟测试 |

不改动的文件：`layers/`, `utils/`, `config.py`, `block_manager.py`, `scheduler.py`, `llm_engine.py`, `sequence.py`

### 验证清单

- [ ] Qwen3 已有 13 个调度测试 + 4 个 BlockManager 测试全部通过
- [ ] LLaMA 模型加载成功（无权重加载 KeyError）
- [ ] Prefill / Decode 正常
- [ ] 生成文本通顺非空
- [ ] temperature=1e-6 下输出确定性
